# =============================
# 🍀 Sistema PlagIA - Limite Local com Filtro de Texto Inteligente (Aprimorado)
# =============================
...
# =============================
st.set_page_config(page_title="PlagIA - PEAS.Co", page_icon="🍀")

st.markdown("""
    <style>
    .main { background-color: #f8f9fa; }
    .stButton>button { background-color: #198754; color: white; font-weight: bold; border-radius: 8px; }
    .stTextInput>div>div>input, .stFileUploader { border: 1px solid #198754; border-radius: 5px; }
...
limite = 4
st.markdown(f"**🔄 Consultas restantes nesta sessão: `{limite - st.session_state['consultas']}`**")

with st.expander("📝 Registro do Usuário", expanded=True):
    ...
    if st.button("🚀 Processar PDF"):
        if not nome or not email:
        ...
        elif st.session_state["consultas"] >= limite:
            st.error("❌ Limite de 4 consultas atingido. Recarregue a página ou adquira acesso premium.")
        else:
            ...
            referencias_sim = [(ref["titulo"], calcular_similaridade(texto_usuario, ref["titulo"] + " " + ref["resumo"]), ref["link"]) for ref in referencias]
            ...
            with open(pdf_path, "rb") as f:
                st.download_button("📥 Baixar Relatório", f, "relatorio_plagio.pdf")
            st.session_state["consultas"] += 1

with st.expander("🔎 Verificação de Autenticidade"):
    codigo_input = st.text_input("🔐 Código de Verificação")
    if st.button("Verificar"):
        if verificar_codigo_google_sheets(codigo_input):
            st.success("✅ Documento autêutico!")
        else:
            st.error("❌ Código inválido.")

with st.expander("💚 Apoie Este Projeto!"):
    st.markdown("""
        <p>Ajude a manter o projeto gratuito com uma doação via <strong>Pix de R$ 20,00</strong>.</p>
    ...
    """, unsafe_allow_html=True)
    qr_img = gerar_qr_code_pix("00020126400014br.gov.bcb.pix0118pesas8810@gmail.com520400005303986540520.005802BR5925PEDRO EMILIO AMADOR SALOM6013TEOFILO OTONI62200516PEASTECHNOLOGIES6304C9DB")
    st.image(qr_img, caption="📲 Escaneie o QR Code para doar via Pix", width=250)
    st.success("🙏 Obrigado pelo apoio!")
