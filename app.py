# =============================
# 🍀 Sistema PlagIA - v10.1 - Correção de Geração de PDF
# Corrige o AttributeError ao gerar o PDF em memória e retornar bytes.
# =============================
...
def load_professional_css() -> str:
    ...
    .hero-header { background: linear-gradient(-45deg, #667eea, #764ba2, #89f7fe, #66a6ff); background-size: 400% 400%; animation: gradientShift 15s ease infinite; padding: 2.5rem; border-radius: 20px; margin-bottom: 2rem; text-align: center; color: white; box-shadow: 0 20px 40px rgba(0,0,0,0.1); }
    @keyframes gradientShift { 0% {background-position: 0% 50%} 50% {background-position: 100% 50%} 100% {background-position: 0% 50%} }
    .glass-card { background: rgba(255, 255, 255, 0.25); backdrop-filter: blur(10px); border-radius: 20px; border: 1px solid rgba(255, 255, 255, 0.18); padding: 2rem; margin-top: 1.5rem; box-shadow: 0 8px 32px 0 rgba(31, 38, 135, 0.15); }
    ...
    .stButton>button:hover { transform: translateY(-3px); box-shadow: 0 6px 20px rgba(102, 126, 234, 0.4); }
    .stDownloadButton>button { background-color: #198754; color: white; }
...
def limpar_texto(texto_bruto):
    ...
    for linha in linhas:
        linha = linha.strip()
        if not linha or len(linha) < 5 or contagem[linha] > 3: continue
        if re.match(r"^Página?\s*\d+$", linha, re.IGNORECASE): continue
        if "doi" in linha.lower() and len(linha) < 50: continue
        if re.search(r"\bResumo\b", linha, re.IGNORECASE): capturar = True
        if capturar: linhas_filtradas.append(linha)
        if re.search(r"\bRefer[eê]ncias\b|\bBibliografia\b", linha, re.IGNORECASE): break
...
# =============================
# 📄 Classe PDF e Geração de Relatório (CORRIGIDO)
# =============================
class PDF(FPDF):
    def header(self):
...
def gerar_relatorio_pdf(referencias_com_similaridade, nome, email, codigo_verificacao):
    ...
    # ##################################################################
    # Gera o PDF em memória e retorna os bytes, que é o formato correto.
